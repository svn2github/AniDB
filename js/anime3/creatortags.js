 ...
 */
jsVersionArray.push({
	"file":"anime3/creatortags.js",
	"version":"1.0",
	"revision":"$Revision: 3019 $",
	"date":"$Date:: 2009-09-01 11:43:05 +0200#$",
...
function fetchData(creatorid) {
	var req = xhttpRequest();
	var extra = "";
	if (creatorid && !isNaN(creatorid) && creatorid > 0) extra = "&creatorid="+creatorid;
	if (''+window.location.hostname == '') xhttpRequestFetch(req, 'xml/'+(extra != '' ? 'creator'+creatorid+'_tags' : 'creatortags')+'.xml', parseData);
	else xhttpRequestFetch(req, 'animedb.pl?show=xmln&t=creatortags'+extra, parseData);
}
...
function prepPage() {
	...
	var creatoridInput = getElementsByName(inputs, 'ctag.creatorid', false)[0];
	var creatorid = (!creatoridInput ? null : creatoridInput.value);
	textArea = div.getElementsByTagName('textarea')[0];
	insertButton = getElementsByName(div.getElementsByTagName('input'), 'do.add', false)[0];
	fetchData(Number(creatorid));
}
