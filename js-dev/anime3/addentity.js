/* @file addcharacter/addcreator page support scripts
 * @author fahrenheit (alka.setzer@gmail.com)
 *         
 * @version 1.0 (17.05.2007)
 */
jsVersionArray.push({
	"file":"anime3/addentity.js",
	"version":"1.5",
	"revision":"$Revision$",
	"date":"$Date::                           $",
	"author":"$Author$",
	"changelog":"Added support for creators"
});
...
function CCharInfo(node) {
	this.charid = Number(node.getAttribute(!isCreatorPage ? 'charid' : 'creatorid'));
...
function CCDesc(node) {
	...
	this.creatortype = (isCreatorPage ? Number(node.getAttribute('creatortype')) : null);
	var type = "Main";
	switch(this.type) {
...
function fetchData(type,searchString) {
	...
	if (!isCreatorPage) {
		...
		if (type == "creatorbyid") {
			if (''+window.location.hostname == '') xhttpRequestFetch(req, 'xml/creator'+searchString+'_desc.xml', prepAjaxSearch);
			else xhttpRequestFetch(req, 'animedb.pl?show=xmln&t=search&type=creatordescbyid&id='+searchString, prepAjaxSearch);
		} else {
			if (''+window.location.hostname == '') xhttpRequestFetch(req, 'xml/creatordescsearch.xml', parseData);
			else xhttpRequestFetch(req, 'animedb.pl?show=xmln&t=search&type=creatordesc&search='+escape(searchString), parseData);
		}
	}
}
...
function prepAjaxSearch(xmldoc) {
	...
	var cdescNodes = root.getElementsByTagName(!isCreatorPage ? 'characterdescbyid' : 'creatordescbyid');
	cids = new Array();
...
function parseData(xmldoc) {
	var root = xmldoc.getElementsByTagName('root').item(0);
	if (!root) { errorAlert('parseData','no root node'); return; }
	var cdescNodes = root.getElementsByTagName(!isCreatorPage ? 'characterdesc' : 'creatordesc');
	cids = new Array();
	for (var d = 0; d < cdescNodes.length; d++) {
		var descNode = new CCDesc(cdescNodes[d]);
		if (cids.indexOf(descNode.charid) < 0) cids.push(descNode.charid);
...
function showResults() {
	...
	if (caption) caption.firstChild.nodeValue = (!isCreatorPage ? "Characters" : "Creators") +" matching \""+searchString+"\"";
...
function doSearch() {
	...
	fetchData(!isCreatorPage ? "chardesc" : "creatordesc",searchString);
}
...
function prepPage() {
	...
	isCreatorPage = (idDiv.className.indexOf('addcreator') >= 0);
	...
	if (newguiseinput.value != "" && !isNaN(Number(newguiseinput.value))) {
		if (!isCreatorPage) fetchData("charbyid",Number(newguiseinput.value));
		else fetchData("creatorbyid",Number(newguiseinput.value));
	}
	...
	caption.appendChild(document.createTextNode(!isCreatorPage ? "Characters" : "Creators"));
