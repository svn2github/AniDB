...
CVBRIHeader* CVBRIHeader::FindHeader(const CMPAFrame* pMPAFrame)
{
	// VBRI header always at fixed offset
	DWORD dwOffset = pMPAFrame->m_dwOffset + MPA_HEADER_SIZE + 32;
	
	// VBRI ID found?
	if (!CheckID(pMPAFrame->m_pMPAStream, dwOffset, 'V', 'B', 'R', 'I'))
		return NULL;
	
	return new CVBRIHeader(pMPAFrame, dwOffset);
}


CVBRIHeader::CVBRIHeader(const CMPAFrame* pMPAFrame, DWORD dwOffset) :
	CVBRHeader(pMPAFrame->m_pMPAStream, dwOffset)
{
	...
	*/
	...
	m_dwVersion = m_pMPAStream->ReadBEValue(2, dwOffset);
	m_fDelay = (float)m_pMPAStream->ReadBEValue(2, dwOffset);
	m_dwQuality = m_pMPAStream->ReadBEValue(2, dwOffset);
	m_dwBytes = m_pMPAStream->ReadBEValue(4, dwOffset);
	m_dwFrames = m_pMPAStream->ReadBEValue(4, dwOffset);
	m_dwTableSize = m_pMPAStream->ReadBEValue(2, dwOffset) + 1;	//!!!
	m_dwTableScale = m_pMPAStream->ReadBEValue(2, dwOffset);
	m_dwBytesPerEntry = m_pMPAStream->ReadBEValue(2, dwOffset);
	m_dwFramesPerEntry = m_pMPAStream->ReadBEValue(2, dwOffset);
	...
	if (m_pnToc)
	{
		...
		{
			m_pnToc[i] = m_pMPAStream->ReadBEValue(m_dwBytesPerEntry, dwOffset);
		}
	}

	// get length of file (needed for seeking)
	m_dwLengthSec = pMPAFrame->m_pMPAHeader->GetLengthSecond(m_dwFrames);
}
