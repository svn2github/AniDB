...
CLyrics3Tag* CLyrics3Tag::FindTag(CMPAStream* pMPAStream, bool bAppended, DWORD dwBegin, DWORD dwEnd)
{
	// stands at the end of file
	DWORD dwOffset = dwEnd - 9;
	BYTE* pBuffer = pMPAStream->ReadBytes(9, dwOffset, false, true);

	// is it Lyrics 2 Tag
	if (memcmp("LYRICS200", pBuffer, 9) == 0)
		return new CLyrics3Tag(pMPAStream, dwOffset, true);
	else if (memcmp("LYRICSEND", pBuffer, 9) == 0)
		return new CLyrics3Tag(pMPAStream, dwOffset, false);
	
	return NULL;
}

CLyrics3Tag::CLyrics3Tag(CMPAStream* pMPAStream, DWORD dwOffset, bool bVersion2) :
	CTag(pMPAStream, _T("Lyrics3"), true, dwOffset)
{
	BYTE* pBuffer;
	if (bVersion2)
	{
		...
		dwOffset -= 6;
		pBuffer = pMPAStream->ReadBytes(6, dwOffset, false); 
	...
	else
	{
		...
		m_dwOffset -= 5100;
		pBuffer = pMPAStream->ReadBytes(11, m_dwOffset, false);
