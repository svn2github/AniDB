...
CID3V2Tag* CID3V2Tag::FindTag(CMPAStream* pMPAStream, bool bAppended, DWORD dwBegin, DWORD dwEnd)
{
	...
	BYTE* pBuffer = pMPAStream->ReadBytes(10, dwOffset, false);

	if (memcmp(szID, pBuffer, 3) == 0)
		return new CID3V2Tag(pMPAStream, bAppended, dwOffset);
	return NULL;
}


CID3V2Tag::CID3V2Tag(CMPAStream* pMPAStream, bool bAppended, DWORD dwOffset) :
	CTag(pMPAStream, _T("ID3"), false, dwOffset)
{
	dwOffset += 3;

	// read out version info
	BYTE* pBuffer = m_pMPAStream->ReadBytes(3, dwOffset);
	SetVersion(2, pBuffer[0], pBuffer[1]);
	BYTE bFlags = pBuffer[3];
	m_bUnsynchronization = bFlags & 0x80;	// bit 7
	m_bExtHeader = bFlags & 0x40;			// bit 6
	m_bExperimental = bFlags & 0x20;		// bit 5
	m_bFooter = bFlags & 0x10;				// bit 4

	// convert synchsafe integer
	m_dwSize = GetSynchsafeInteger(m_pMPAStream->ReadBEValue(4, dwOffset)) + (m_bFooter?20:10);
