...
//---------------------------------------------------------------------------
void File_Id3v2::Read_Buffer_Init()
{
    Id3v2_Size=0;
}

//---------------------------------------------------------------------------
void File_Id3v2::Read_Buffer_Continue()
{
    //ID3v2 present?
    if (Id3v2_Size==0)
    {
...
bool File_Id3v2::Buffer_Parse()
{
    ...
    switch (Id3v2_Version)
    {
        ...
        default : return false; //Big problem if here...
    }
    Element_Next=File_Offset+Buffer_Offset+10+Element_Size;
...
//---------------------------------------------------------------------------
void File_Id3v2::Header_Extended()
{
    int32u Size;
    CC_BEGIN();
    GET_B4 (Size,                                               Size);
...
void File_Id3v2::Padding()
{
    ...
    if (Padding_Size>0)
    {
        ELEMENT(0, "Padding", Padding_Size);
        FLUSH();
    }
...
void File_Id3v2::T__X()
{
    ...
    switch (Encoding)
    {
        case 0 : GET_LOCAL (Element_Size-Stream_Pos, Element_Values(1), The_actual_text); break;
        case 1 : GET_UTF16 (Element_Size-Stream_Pos, Element_Values(1), The_actual_text); break;
        case 2 : GET_UTF16B(Element_Size-Stream_Pos, Element_Values(1), The_actual_text); break;
        case 3 : GET_UTF8  (Element_Size-Stream_Pos, Element_Values(1), The_actual_text); break;
        default : ;
    }
}
...
void File_Id3v2::W__X()
{
    ...
    if (Encoding==1)
        ...
        Stream_Pos+=Element_Values(0).size()+1;   //UTF-8 NULL
    GET_LOCAL(Element_Size-Stream_Pos, Element_Values(1),       URL);
}
...
void File_Id3v2::APIC()
{
    ...
    GET_LOCAL(Element_Size-Stream_Pos, Description,             Description);
...
File_Id3v2_Helper::File_Id3v2_Helper(File__Base* Base_)
{
    #if defined(MEDIAINFO_ID3_YES)
        Base=Base_;
        Id3v2_Size=0;
        Id3v2_Size_Continue=0;
        ID3v2=NULL;
    #endif //ID3
}

File_Id3v2_Helper::~File_Id3v2_Helper()
{
    #if defined(MEDIAINFO_ID3_YES)
...
bool File_Id3v2_Helper::Id3v2_Read_Buffer_Continue()
{
    if (Id3v2_Size_Continue==0)
    {
        ...
        if (CC3(Base->Buffer+Base->Buffer_Offset)==CC3("ID3"))
        {
            ...
            if (Base->Buffer_Offset+10>Base->Buffer_Size)
                return false;
            Id3v2_Size=SynchSafeInt(Base->Buffer+6)+10;
            Id3v2_Size_Continue=Id3v2_Size;
        }
        else
            return true; //Nothing

        //Details
        Base->ELEMENT(0, "ID3v2 tags", Id3v2_Size);
        Base->FLUSH();

        //Moving MaximumOffset
        Base->File_MaximumOffset=Id3v2_Size+Base->File_MaximumOffset;
    }

    //If we continue the Id3v2 tag
    #if defined(MEDIAINFO_ID3V2_YES)
    if (ID3v2==NULL)
    {
        ID3v2=new File_Id3v2;
        Base->Open_Buffer_Init(ID3v2);

    }
    if (Id3v2_Size_Continue>0)
    {
        if (Id3v2_Size_Continue<=Base->Buffer_Size)
        {
            Base->Open_Buffer_Continue(ID3v2, Base->Buffer+Base->Buffer_Offset, Id3v2_Size_Continue);
            Base->Buffer_Offset+=Id3v2_Size_Continue;
            Base->Open_Buffer_Finalize(ID3v2);
            Id3v2_Size_Continue=0; //We don't need it anymore
        }
        else
        {
            Base->Open_Buffer_Continue(ID3v2, Base->Buffer+Base->Buffer_Offset, Base->Buffer_Size-Base->Buffer_Offset);
            Id3v2_Size_Continue-=Base->Buffer_Size-Base->Buffer_Offset;
            Base->Buffer_Offset=Base->Buffer_Size;
            return false; //Need more bytes
        }
    }
    #else
        Base->Buffer_Offset+=Id3v2_Size;
    #endif //ID3V2

    return true;
}

void File_Id3v2_Helper::Id3v2_Read_Buffer_Finalize()
{
    #if defined(MEDIAINFO_ID3_YES)
    if (ID3v2!=NULL)
    {
