...
//---------------------------------------------------------------------------
const Char*  MediaInfo_Version=_T("MediaInfoLib - v0.7.5.3");
...
bool MediaInfo_Config::File_Filter_Get (const int16u Value)
{
    Enter();
    //Test
    bool Exists=(File_Filter_16.find(Value)!=File_Filter_16.end());
    Leave();
    return Exists;
}
...
{
    //Example: "451;memory://0x00000000:1316"
    Enter(true);
    ...
    if (Target.find(_T("memory://"))==0)
    {
        if (Target.size()>9 && Target.find(_T("memory://"))==0 && Target.find(_T(":"), 9)!=string::npos)
        {
            ...
            File_Duplicate[Code].Method=file_duplicate::method_buffer_nodelete;
            File_Duplicate[Code].Buffer=(char*)Address.To_int64u();
            File_Duplicate[Code].Buffer_Size_Max=(size_t)Size.To_int64u();
        }
        else
            File_Duplicate[Code].Method=file_duplicate::method_buffer;
    }
    else if (Target.size()>7 && Target.find(_T("file://"))==0)
    {
        if (Target.size()>7)
        {
            File_Duplicate[Code].Method=file_duplicate::method_filename;
            File_Duplicate[Code].FileName=Target.substr(7, string::npos);
        }
    }
    else
        File_Duplicate[Code].Method=file_duplicate::method_filename_ID;
    File_Duplicate_16.clear();
    Leave();
}

bool MediaInfo_Config::File_Duplicate_Get (const Ztring &Value)
{
    Enter();
    bool Exists=(File_Duplicate.find(Value)!=File_Duplicate.end());
    Leave();
    return Exists;
}

bool MediaInfo_Config::File_Duplicate_Get (const int16u Value)
{
    Enter();
    //Filling if not yet done
    if (File_Duplicate_16.empty())
    {
        std::map<Ztring, file_duplicate>::iterator Temp=File_Duplicate.begin();
        while (Temp!=File_Duplicate.end())
        {
            File_Duplicate_16[Temp->first.To_int16u()]=&Temp->second;
            Temp++;
        }
    }

    //Test
    bool Exists=(File_Duplicate_16.find(Value)!=File_Duplicate_16.end());
    Leave();
    return Exists;
}

bool MediaInfo_Config::File_Duplicate_Get ()
{
    Enter();
    bool Exist=!File_Duplicate.empty();
    Leave();
    return Exist;
}

bool MediaInfo_Config::File_Duplicate_Buffer_NoDelete_Requested (const int16u Value)
{
    Enter();
    //Filling if not yet done
    if (File_Duplicate_16.empty())
    ...
    bool Requested=File_Duplicate_16[Value]->Method==file_duplicate::method_buffer_nodelete;
    Leave();
    return Requested;
}

int8u* MediaInfo_Config::File_Duplicate_Buffer_NoDelete_Buffer (const int16u Value)
{
    Enter();
    //Filling if not yet done
    if (File_Duplicate_16.empty())
    ...
    int8u* Buffer=(int8u*)File_Duplicate_16[Value]->Buffer;;
    Leave();
    return Buffer;
}

size_t MediaInfo_Config::File_Duplicate_Buffer_NoDelete_Buffer_Size (const int16u Value)
{
    Enter();
    //Filling if not yet done
    if (File_Duplicate_16.empty())
    ...
    size_t Buffer_Size=Config.File_Duplicate_16[Value]->Buffer_Size_Max;
    Leave();
    return Buffer_Size;
}
