...
void File_Mk::Read_File()
{
    ...
    try {
    #ifdef _WIN32
        WinIOCallback File(File_Name.c_str(), MODE_READ);
    #else
        StdIOCallback File(File_Name.To_Local().c_str(), MODE_READ); //TODO : Unicode FileName
    #endif
    ...
    if (ElementLevel[0] && EbmlId(*ElementLevel[0])==KaxSegment::ClassInfos.GlobalId)
    {
        ...
        SUB(Attachments)
            ...
            SUB(Attached)
                ...
                BEGIN
                KAX(FileName)
                    if (CoverName.empty()) CoverName.From_Unicode(UTFstring(*Kax).c_str()); //FileName is not a priority
                KAX(FileDescription)
                    CoverName.From_Unicode(UTFstring(*Kax).c_str());
                END
                if (!CoverName.empty())
                {
            ...
            SUB(EditionEntry)
                ...
                SUB(ChapterAtom)
                    ...
                    SUB(ChapterDisplay)
                        BEGIN
                        KAX(ChapterString)
                            Chapters[Chapters_Pos](Ztring::ToZtring(Pos))+=Ztring().From_Unicode(UTFstring(*Kax).c_str());
                        KAX(ChapterLanguage)
        ...
        SUB(Tags)
            ...
            SUB(Tag)
                ...
                SUB(TagSimple)
                    ...
                    BEGIN
                    KAX(TagName)
                        Name.From_Unicode(UTFstring(*Kax).c_str());
                    KAX(TagString)
                        General[0](Name).From_Unicode(UTFstring(*Kax).c_str());
                    END
                END
            END
        END
