...
int MediaInfo::Format_Test_FillBuffer_Init()
{
    ...
    //FileSize
    File_Size=File_Handle->Size_Get();
...
int MediaInfo::Format_Test_FillBuffer_Continue()
{
    ...
    {
        if (File_Handle->GoTo(Info->File_GoTo))
        {
    ...
    {
        Buffer_Size=File_Handle->Read(Buffer, Buffer_Size_Max);
        if (Buffer_Size!=0)
        {
            //Read is OK
            if (Buffer_Size==0)
...
size_t MediaInfo::Open_Buffer_Continue (const int8u* ToAdd, size_t ToAdd_Size)
{
    ...
    if (Info!=NULL && Info->File_GoTo!=(int64u)-1 && Info->File_Offset<Info->File_GoTo)
    {
        #ifndef MEDIAINFO_MINIMIZESIZE
            if (Config.Details_Get())
            {
    ...
    //Enough?
    //if (File_Offset+Buffer_Size<File_Size && Buffer_Size<Buffer_NormalSize)
    ...
    {
        delete Info; Info=NULL;
        if (Buffer_Size>=Buffer_NormalSize)
        {
            //Reserving unused data
            memmove(Buffer, Buffer+Buffer_Size-Buffer_NormalSize, Buffer_Size-Buffer_NormalSize);
            Buffer_Size-=Buffer_NormalSize;
            File_Offset+=Buffer_NormalSize;
        }
    }
    */
    ...
    else
    {
        ...
        if (Buffer==NULL)
        {
            ...
            //Copying buffer
            memcpy(Buffer, ToAdd, ToAdd_Size);
        }
...
//---------------------------------------------------------------------------
size_t MediaInfo::Output_Buffer_Get (const Ztring &Value, char** Output_Buffer)
{
    if (!Info)
    {
        if (Output_Buffer)
