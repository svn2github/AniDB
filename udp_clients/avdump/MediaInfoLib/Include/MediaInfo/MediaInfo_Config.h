...
public :
    ...
    const Ztring   &EmptyString_Get() const; //Use it when we can't return a reference to a true string
          ...
          bool      File_Duplicate_Get  (const Ztring &Value);
          bool      File_Duplicate_Get  (const int16u  Value);
          bool      File_Duplicate_Get  ();
          bool      File_Duplicate_Buffer_NoDelete_Requested    (const int16u Value);
          int8u*    File_Duplicate_Buffer_NoDelete_Buffer       (const int16u Value);
          size_t    File_Duplicate_Buffer_NoDelete_Buffer_Size  (const int16u Value);
...
private :
    ...
    struct file_duplicate
    {
        enum method
        {
            ...
            method_filename,
            method_filename_ID, //FileName is the ID.
        };
        method  Method;
        char*   Buffer;
        size_t  Buffer_Size;
    ...
    std::map<Ztring, file_duplicate >   File_Duplicate;
    std::map<int16u, file_duplicate*>   File_Duplicate_16;
};
