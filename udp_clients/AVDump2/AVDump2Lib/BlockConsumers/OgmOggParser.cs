...
namespace AVDump2Lib.BlockConsumers {
		...
		internal void Parse(FileSource dataSrc) {
			...
			while(!dataSrc.EOF()) {
				...
				if(Tracks.Contains(pageHeader.StreamId)) {
					...
					} else if(Encoding.ASCII.GetString(header, 0, 7).Equals("fishead") || Encoding.ASCII.GetString(header, 1, 4).Equals("kate")) {
						track = null;
					} else {
	...
	public class VideoTrack : Track {
		...
		public double? PAR { get; private set; }
		...
		public VideoTrack(uint streamId, object codecInfo)
			...
			if(codecInfo is OGMVideo) {
				...
				Width = info.Width;
				Height = info.Height;
			} else if(((byte[])codecInfo).Length == 42 && Encoding.ASCII.GetString(((byte[])codecInfo), 1, 6).Equals("theora")) {
				var header = (byte[])codecInfo;
				codecName = "Theora";
				codecVersion = header[7] + "." + header[8] + "." + header[9];
				Width = header[14] << 16 | header[15] << 8 | header[16];
				Height = header[17] << 16 | header[18] << 8 | header[19];
				FrameRate = (header[22] << 24 | header[23] << 16 | header[24] << 8 | header[25]) / (double)(header[26] << 24 | header[27] << 16 | header[28] << 8 | header[29]);
		...
		internal override void Finish() {
			if(codecInfo is OGMVideo) {
				...
				//duration = TimeSpan.FromMilliseconds(FrameCount / FrameRate);
				//bitrate = Length / duration.Value.TotalSeconds;
			}
		}
		...
		public static object GetCodecInfo(byte[] header) {
			...
			if(header.Length == 0x39 && Encoding.ASCII.GetString(header, 1, 5).Equals("video")) {
				codecInfo = GetStruct<OGMVideo>(header, 1, 0x38);
			} else if(header.Length == 42 && Encoding.ASCII.GetString(header, 1, 6).Equals("theora")) {
				codecInfo = header;
			}
	...
	public class AudioTrack : Track {
		...
		public AudioTrack(uint streamId, object codecInfo)
			...
			if(codecInfo is VorbisIdentHeader) {
				...
				//    ChannelCount = info.ChannelCount;
				//    SampleRate = info.SamplesPerUnit;
			} else if(((byte[])codecInfo).Length >= 79 && System.Text.Encoding.ASCII.GetString(((byte[])codecInfo), 1, 4).Equals("FLAC")) {
				var header = (byte[])codecInfo;
				codecName = "Flac";
				SampleRate = header[33] << 12 | header[34] << 4 | (header[35] & 0xF0) >> 4;
				ChannelCount = (header[35] & 0x0E) >> 1;
				//SampleCount = (header[35] & 0x01) << 4 | (header[36] & 0xF0) >> 4;
			}
		}
		...
		public static object GetCodecInfo(byte[] header) {
			...
			} else if(header.Length >= 79 && System.Text.Encoding.ASCII.GetString(header, 1, 4).Equals("FLAC")) {
				codecInfo = header;
			} else if(header.Length >= 46 && System.Text.Encoding.ASCII.GetString(header, 1, 5).Equals("audio")) {
				codecInfo = GetStruct<OGMAudio>(header, 1, 56);
