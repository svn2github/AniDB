...
'''The following code is pure-Python MD4 and is used if no PCT is present.
This code is LGPL, and was written by Dmitry Rozmanov. I have only changed the
code in order to be more Pythonic and to reduce total lines of code.'''
# Classes U32 and MD4 copyright (C) 2001-2002 Dmitry Rozmanov
...
class MD4:
    ...
    def digest(self):
        ...
        temp.update(''.join([chr(i) for i in padding[:int(padlen)]]))
        ...
        temp.update(''.join([chr(i) for i in s]))
