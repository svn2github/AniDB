...
function cTimeDate(data) {
	...
	if (data.indexOf(' ') >= 0) {
		...
		if (data.indexOf('-') >= 0) {
			var dash = data.split('-');
			data = dash[2] + '.' + dash[1] + '.' + dash[0];
		} else {
			var dot = data.split('.');
			data = dot[0] + '.' + dot[1] + '.' + (dot[2].length > 2 ? dot[2].substring(2,4) : dot[2]);
...
function createLabelCheckbox(name,id,checked,text) {
	if (!id && !text) return createCheckbox(name,checked);
	var ck = document.createElement('input');
	ck.type = 'checkbox';
	ck.name = name;
	if (id) ck.id = id;
	if (!id && text) ck.title = text;
	if (checked != null) ck.checked = checked;
	if (id && text) {
