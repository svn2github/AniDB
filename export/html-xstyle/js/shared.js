...
var mylist = new Array();		// Mylist thingies
...
dragDrop = {
	...
	initElement: function (element, text, smallText) {
		if (typeof element == 'string')
			element = document.getElementById(element);
		...
		h3.onclick = function() {
			...
			if (!content) return;
			var num = 0;
			if (content.style.visibility != 'hidden') num = Number(this.parentNode.clientHeight - content.clientHeight - 10);
			else num = Number(this.parentNode.clientHeight + content.clientHeight + 10);
		...
		// anchor
		var a = createIcon(null, '[x]', '#', function() { this.parentNode.style.display = 'none'; }, 'Close', 'i_close');
