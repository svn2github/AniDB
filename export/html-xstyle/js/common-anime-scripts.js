...
var genAnimeCols = [{'name':"expand",'classname':"expand",'header':"X",'abbr':"Expand/fold"},
					...
					{'name':"title",'classname':"title",'header':"Title",'sort':"c_setlatin"},
					{'name':"eps",'classname':"stats eps",'header':"Eps",'headclass':"eps",'sort':"c_set"},
					{'name':"seen",'classname':"stats seen",'header':"Seen",'headclass':"seen",'sort':"c_set"},
					{'name':"rating",'classname':"rating anime mid",'header':"Rating",'headclass':"rating",'sort':"c_set"},
					{'name':"mylist",'classname':"vote",'header':"My",'headclass':"my",'sort':"c_set"},
					{'name':"reviews",'classname':"rating attavg mid",'header':"Reviews",'headclass':"attavg",'sort':"c_set"}	];
...
/* This function creates and returns an Object with all the possible icons for a given anime
 * @param anime Group to build icon object
...
/* Function that creates an anime table row
 * @param aid Anime ID
 ...
 * @return anime row
 */
function createAnimeRow(aid,cols,skips) {
	var row = document.createElement('tr');
	row.id = 'a'+aid;
	var anime = animes[aid];
	var icons = createAnimeIcons(anime); // get icons
	for (var c = 0; c < cols.length; c++) {
		...
		// expand, check, title, eps, seen, rating, mylist, reviews
		switch(col['name']) {
			case 'expand':
				createCell(row, col['classname'], icons['expand'], null, colSpan);
				break;
			case 'check':
				var ck = createCheckBox(null,'myamod.a.'+anime.id,'myamod.a.'+anime.id,null,false);
				createCell(row, col['classname'], ck, null, colSpan);
				break;
			case 'title':
				var title;
				var titleURL = 'anime.html?aid='+aid;
				var altTitle = jaTitle = curTitle = '';
				if (!anime.titles['official']) altTitle = jaTitle = '';
				else {
					if (animeTitleLang != animeAltTitleLang && 
						anime.titles['official'][animeAltTitleLang] && 
						anime.titles['official'][animeAltTitleLang] != curTitle) altTitle = anime.titles['official'][animeAltTitleLang];
					if (animeTitleLang != 'ja' &&
						animeAltTitleLang != 'ja' &&
						anime.titles['official']['ja'] && 
						anime.titles['official']['ja'] != altTitle) jaTitle = anime.titles['official']['ja'];
				}
				if (altTitle != '' || jaTitle != '') {
					if (animeTitleDisplay == 1 || (animeTitleDisplay == 4 && jaTitle == '')) 
						title = createTextLink(null, anime.getOfficialTitle()+' ('+altTitle+')', titleURL, null, null, null, null);
					if (animeTitleDisplay == 2 || (animeTitleDisplay == 3 && jaTitle == '')) {
						var tooltip = (altTitle != '') ? mapLanguage(animeAltTitleLang) + ' title: '+ altTitle : '';
						title = createTextLink(null, anime.getOfficialTitle(), titleURL, null, null, tooltip, null);
					}
					if (animeTitleDisplay == 3 && jaTitle != '') {
						var titleTag = '';
						if (altTitle != '') titleTag = mapLanguage(animeAltTitleLang) + ' title: '+ altTitle + ' / ';
						title = createTextLink(null, anime.getOfficialTitle(), titleURL, null, null, titleTag += jaTitle, null);
					}
					if (animeTitleDisplay == 4 && jaTitle != '') {
						var titleTag = ' (';
						if (altTitle != '') titleTag += altTitle+' / ';
						title = createTextLink(null, anime.getOfficialTitle()+ titleTag +jaTitle+ ')', titleURL, null, null, null, null);
					}
				} else title = createTextLink(null, anime.title, titleURL, null, null, null, null);
				if (icons['state'] || icons['type']) {
					var span = document.createElement('span');
					span.className = 'icons';
					if (icons['wishlist']) span.appendChild(icons['wishlist']);
					if (icons['awards']) span.appendChild(icons['awards']);
					if (icons['status']) span.appendChild(icons['status']);
					if (icons['type']) span.appendChild(icons['type']);
					var cell = createCell(null, col['classname'], span, anime.getOfficialTitle(), colSpan);
					cell.appendChild(title);
					row.appendChild(cell);
				} else createCell(row, col['classname'], title, anime.getOfficialTitle(), colSpan);
				break;
			case 'eps':
				var txt = anime.neps['user'] + '/' + anime.neps['cnt']+(anime.seps['user'] ? '+' + anime.seps['user'] : '');
				createCell(row, col['classname'], document.createTextNode(txt), anime.neps['user'], colSpan);
				break;
			case 'seen':
				var txt = anime.neps['seen'] + '/' + anime.neps['cnt']+(anime.seps['seen'] ? '+' + anime.seps['seen'] : '');
				createCell(row, col['classname'], document.createTextNode(txt), anime.neps['seen'], colSpan);
				break;
			case 'rating':
				var txt = (anime.rating['votes'] ? anime.rating['rating'] : '-');
				var cell = createCell(null, col['classname'], document.createTextNode(txt), (anime.rating['votes'] ? anime.rating['rating'] : '0'), colSpan);
				var span = document.createElement('span');
			...
			case 'mylist':
				...
				createCell(row, col['classname'], document.createTextNode(vote), (anime.myvote ? anime.myvote['vote'] : '0'), colSpan);
				break;
			case 'reviews':
				var txt = (anime.reviews ? anime.rrating : '-');
				var cell = createCell(null, col['classname'], document.createTextNode(txt), (anime.reviews ? anime.reviews : '0'), colSpan);
				var span = document.createElement('span');
				span.className = 'count';
				...
				break;
			default:
				errorAlert('createAnimeRow','unknown column with name: '+col['name']);
		}
	}
	return row;
}
	...
	for (var c = 0; c < cols.length; c++) {
		...
		switch(col['name']) {
			case 'expand':
				...
				break;
			case 'epno':
				var epno = createTextLink(null, episode.epno, 'animedb.pl?show=ep&eid='+eid, null, null, null, null);
