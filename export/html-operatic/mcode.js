function writeSpans(){
	...
	if (curpage < 1){
		document.getElementById("prev").innerHTML = NspanPrev;
		document.getElementById("prev1").innerHTML = NspanPrev;
	} else {
	...
	if (numpages > 1){
		...
		document.getElementById("next").innerHTML = NspanNext;
		document.getElementById("next1").innerHTML = NspanNext;	
	}
}
	...
	if (vis.length>0){
		...
		for (i = vids; i < nids; i++){
			...
			if ( aToExpand == anime.id ){
				output += '<a href="mylist.htm"><img src="images/colapse.gif" title="colapse" alt="colapse" border="0"></a>'
			} else {
				output += '<a href="mylist.htm?=a'+anime.id+'&amp;expand=1"><img src="images/expand.gif" title="expand" alt="expand" border="0"></a>' 
			}
